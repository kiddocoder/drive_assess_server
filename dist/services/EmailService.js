...
class EmailService {
    ...
    async sendPasswordResetEmail(email, name, token) {
        ...
        `,
            ...
            Logger_1.Logger.error("Send password reset email error:", error);
            throw error;
        }
    }
    async sendTestCompletionEmail(email, name, testTitle, score, passed) {
        try {
            const mailOptions = {
                from: `"DriveReady" <${process.env.SMTP_USER}>`,
                to: email,
        ...
        `,
            ...
            await this.transporter.sendMail(mailOptions);
            Logger_1.Logger.info(`Test completion email sent to: ${email}`);
        }
        catch (error) {
            Logger_1.Logger.error("Send test completion email error:", error);
            throw error;
        }
    }
}
